...
public class Register extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(bl){
			...
			request.setAttribute("errormsg","<script language='javascript'>alert('两次密码输入不一致，请重新注册!')</script>");
		    request.getRequestDispatcher("chat/index.jsp").forward(request, response);
		}
		else{
		...
		if(flag){
			request.setAttribute("succmsg","<script language='javascript'>alert('注册成功!')</script>");
		    request.getRequestDispatcher("chat/index.jsp").forward(request, response);
